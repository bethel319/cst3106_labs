...
// Function to roll dice by fetching values from the server
function rollDice() {
    // Fetch dice values from the server
    fetch("http://localhost:3000/roll-dices") // Update with the correct server URL
        .then(response => response.json())
        .then(serverDice => {
            // Update only dice that are not held
            ...
            console.log("Rolled Dice from Server:", dice); // Debugging to confirm dice values
            updateScore(); // Update scores after rolling
        })
        .catch(error => {
            console.error("Error fetching dice values from the server:", error);
            alert("Failed to roll dice. Please check your connection to the server.");
        });
}
